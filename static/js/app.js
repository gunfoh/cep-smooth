...
function App() {
    ...
    const addReport = async (report) => {
        try {
            ...
            // Update state with the report confirmed by the server
...
// --- UI Components (largely unchanged, only pasted one for brevity) ---
...
function HeatmapView({ issues, onBack, initialCoords }) {
    ...
    return (
        <div className="bg-white p-4 rounded-xl shadow-2xl w-full max-w-4xl">
             <div className="flex justify-between items-center mb-4">
                <h2 className="text-2xl font-bold text-gray-800">Issue Heatmap</h2>
                <button onClick={onBack} className="bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded-lg transition-colors">Back</button>
            </div>
            <div id="heatmap-map" className="h-[65vh] w-full rounded-lg shadow-inner bg-gray-200"></div>
        </div>
    );
}
